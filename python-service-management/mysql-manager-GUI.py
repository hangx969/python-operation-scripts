...
# -*- coding: utf-8 -*-
"""
MySQL 数据库服务远程管理图形界面工具

功能描述:
    本脚本提供基于 Tkinter 的图形用户界面，用于远程管理 MySQL 数据库服务。
    通过 SSH 连接到远程服务器，实现 MySQL 服务的启动、停止、状态查询操作，
    并提供滚动式日志显示区域，为数据库运维人员提供便捷的服务管理工具。

技术实现:
    - 使用 Tkinter 和 scrolledtext 构建桌面图形界面
    - 使用 paramiko 库建立 SSH 连接和远程命令执行
    - 采用 systemctl 命令管理 MySQL 系统服务
    - 使用进程检查验证数据库服务运行状态
    - 通过 ScrolledText 组件提供可滚动的日志显示

界面组件设计:
    1. 启动按钮 - 执行 MySQL 服务启动命令
    2. 停止按钮 - 执行 MySQL 服务停止命令
    3. 状态检查按钮 - 查询 MySQL 进程运行状态
