...
功能描述:
    本脚本提供基于 Tkinter 的图形用户界面，用于远程管理 Tomcat 应用服务器。
    通过 SSH 连接到远程服务器，实现 Tomcat 服务的启动、停止、状态查询操作，
    并提供滚动式日志显示区域，为 Java Web 应用运维人员提供便捷的服务管理工具。

技术实现:
    - 使用 Tkinter 和 scrolledtext 构建桌面图形界面
    - 使用 paramiko 库建立 SSH 连接和远程命令执行
    - 采用 Tomcat 官方脚本 (catalina.sh) 管理服务
    - 使用进程检查 (ps -ef) 验证服务运行状态
    - 通过 ScrolledText 组件提供可滚动的日志显示

界面组件设计:
    1. 启动按钮 - 执行 Tomcat 启动脚本 (catalina.sh start)
    2. 停止按钮 - 执行 Tomcat 停止脚本 (catalina.sh stop)
    3. 状态检查按钮 - 查询 Tomcat 进程运行状态
    4. 滚动日志区域 - 显示操作结果和系统反馈信息
    5. 退出按钮 - 关闭应用程序
